# Allianne Matthew's Portfolio

## About Me

I’m Allianne Matthew Dela Vega — a Cyber Systems major at the U.S. Coast Guard Academy graduating in 2026, specializing in **practical cybersecurity engineering, network defense, and maritime cyber operations**.

---

## Cybersecurity & Technical Focus Areas

- **Network Defense & Detection Engineering**  
  Zeek, Suricata, Splunk, IDS/IPS workflows, Windows Event Forwarding

- **Security Analysis & Incident Response**  
  Wireshark, Log triage, event correlation, packet analysis, threat identification

- **Maritime Cyber & SDR Research**  
  AIS decoding, RF signal analysis, NMEA parsing, ArcGIS-based visualization

- **Software & Systems Engineering**  
  Python, C/C++, Bash, Linux, Virtualization, Database Systems, Algorithms

- **Cyber Research for Operational Impact**  
  DoD Research Challenge, AI-enabled interdiction operations, risk frameworks

---

## Capstone Project  
**Enhancing Coast Guard Remote Radio Tower Sites Using SDR (USCG C5I Service Center)**  
Designed a deployable SDR solution using a **USRP B205mini** to consolidate multiple legacy VHF radios into a single, software-defined platform for remote Alaskan sites.  

**Technical highlights:**
- Built an SDR pipeline capable of reproducing conventional radio features  
- Reduced hardware footprint, maintenance load, and power usage  
- Integrated AIS → NMEA decoding into a real-time ArcGIS Pro operational picture  
- Implemented MMSI de-duplication, course/speed tracking, and vessel visualization  

> This capstone demonstrates ability to build full-stack technical systems: RF → signal processing → data parsing → geospatial intelligence.

---

## Research Highlights

### **Firewalls & Foghorns: Cybersecurity in Small & Medium Maritime Operators**  
*2025 DoD Academic Research Challenge – Research Author*  
- Analyzed cyber adoption barriers in the Maritime Transportation System  
- Evaluated DSTs using NIST CSF + DHS/SBA maritime guidance  
- Proposed governance, workforce, and resilience strategies for resource-limited operators  

### **Digital Transformation of USCG Drug Interdiction Operations**  
*School of Leadership & Management – Research Author*  
- Assessed AI, UAV/UUV, and sensor networks for interdiction modernization  
- Integrated AI-driven anomaly detection with GEOINT/OSINT fusion  
- Produced a scalable operational framework for mission alignment  

> These research efforts show strong analytical writing, strategic thinking, and ability to bridge emerging tech with operational missions.

---

## Technical Skills

**Cybersecurity:**  
Network security, IDS/IPS, incident response, vulnerability analysis, malware detection concepts  

**Tools & Platforms:**  
Wireshark • Zeek • Suricata • Splunk • VirtualBox • Git/GitHub  

**Networking & Systems:**  
TCP/IP • DNS • Firewalls • Windows & Linux Administration  

**Programming & Data:**  
Python • C/C++ • Bash • SQL  

**Operating Systems:**  
Windows • Kali Linux • Ubuntu  

**Languages:**  
Fluent in Tagalog  

---

## Professional Experience

### **USCGC Juniper — Honolulu, HI**  
*Crew Member, Summer 2025*  
- Gained operational exposure during weapons training and aids-to-navigation duties  
- Served as Public Affairs Officer for ceremonies and unit engagements  

### **USCG Sector Jacksonville — Incident Management Division**  
*Summer 2025*  
- Conducted domestic Navy tug inspection as part of the Sector Inspection Team
- Observed and aided in emergency Hazardous Material Management (HAZMAT) response at Jacksonville 
Port Authority (JAXPORT)  

---

## Leadership & Service

### **Vice President & President (May 2024 - Present), International Council – USCGA**  
• I lead the international cadet community at the Academy, representing their interests and serving as the primary bridge between cadets and Academy leadership. In this role, I manage logistics, planning, and strategy for multiple major events throughout the year while ensuring clear and professional communication channels.

I coordinate scheduling, resources, and funding for international programs and cultural initiatives—overseeing more than $14,000 in allocated resources. I plan and execute professional development engagements, cultural programs, and morale-building events involving Academy leadership, alumni, and external partner organizations.

Through this work, I ensure international cadets are supported academically, professionally, and personally, while strengthening the Academy’s global engagement and community presence.

### **Operations Liaison (Division Officer), Charlie Company**  
• Managed cadet qualification documentation, personnel readiness, and operational coordination.
• Coordinated with multiple stakeholders to ensure compliance and readiness
• Strengthened attention to detail and process managament.

---

## Certifications
- Google Cybersecurity Professional Certificate
- Red Team Leaders: Certified Cybersecurity Educator Professional

---

## Education
- Bachelor of Science in Cyber Systems under the United States Coast Guard Academy
- Computing Accreditation Commission of ABET accreditation under New England Commission of Higher Education (NECHE)

---

## Connect With Me

**LinkedIn:** www.linkedin.com/in/amdelavega  
**Email:** alliannematthew.z.delavega@uscga.edu  

---

> If you want to see what I’m currently building, check the **Projects** folder — it contains real IDS/IPS deployments, network detection labs, and applied cyber engineering work.
